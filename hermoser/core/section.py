...
class Section:
    ...
    def generate_section(self):
        ...
        self.duration = (sum(intervals)*self.ru) + self.events_list[-1].u if len(self.events_list)>0 else 0

class SetOrientedSection(Section):
    def __init__(self, event_set, rhythmic_unit=1, offset=0):
    ...
    def generate_offsets(self):
        if len(self.event_set.timepoints)==0:
            ...
            i=0
            intervals = []
            while(i<len(self.event_set)):
                actual = rd.choice(list(self.event_set.timepoints))
                triplets = set([1/3, 2/3])

                if actual in triplets:
                    if len(self.event_set)-i>=3:
                        intervals += [actual, actual, actual]
                        i+=3
                    elif len(self.event_set.timepoints - triplets)>0:
                        actual = rd.choice(list(self.event_set.timepoints - triplets))
                        intervals.append(actual)
                        i+=1
                    else:
                        intervals += [1]*(len(self.event_set)-i)
                        i = len(self.event_set)
                else:
                    intervals.append(actual)
                    i+=1
...
class HermaSection(Section):
    ...
    def generate_section(self):
        ...
        while(len(time_points)>0):
            elements = copy.deepcopy([e for e in self.event_set])
