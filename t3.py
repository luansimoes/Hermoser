...
def random_sets_with_interval_constraints(bag=list(range(-39, 49, 1)), sizes=[26,21,25], max_value=1, base=1.265):
    r = DynSet([SonicEvent(x, 80, 0) for x in bag], base=base)
    ...
    a = DynSet(sets[0], base=base)
    b = DynSet(sets[1], base=base)
    c = DynSet(sets[2], base=base)
...
piece = HermaLikePiece("tests/octave_3-1.mid", tempo=tempo)
...
piece = HermaLikePiece("tests/two_octave_3-1.mid", tempo=tempo)
piece.set_sections(sections)
piece.run_composer(sequential=True)
