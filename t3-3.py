...
def random_sets_with_interval_constraints(bag=list(range(-39, 49, 1)), sizes=[26,21,25], max_value=1, base=1.265, k=12):
    d_vector = set(list(range(k)))
    r = DynSet([SonicEvent(x, 80, 0) for x in bag], base=base, dyn_vector=d_vector)
    ...
    d_vectors = [set(rd.choices(list(range(k)), k=rd.randint(5, 16))) for i in range(3)]
    a = DynSet(sets[0], base=base, dyn_vector=d_vectors[0])
    b = DynSet(sets[1], base=base, dyn_vector=d_vectors[1])
    c = DynSet(sets[2], base=base, dyn_vector=d_vectors[2])
...
print_messages()
two_octave_sets = random_sets_with_interval_constraints(two_octave, sizes=[10,10,10], base=2, k=24)
print("-------------------------------------------")
print_messages()
...
sections = [
        ...
        Section(a+b+c,0.125, temporal_rule=lambda : rd.randint(1, 8)),
    ]
    
tempo=60

piece = HermaLikePiece("tests/octave_3-3.mid", tempo=tempo)
...
piece = HermaLikePiece("tests/two_octave_3-3.mid", tempo=tempo)
piece.set_sections(sections)
piece.run_composer(sequential=True)
